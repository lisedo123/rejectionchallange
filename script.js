...
async function loadState() {
    try{
        const res = await window.Storage.length('rejection-log-state');
        if(res && res.value){
...
function render() {
    ...
    document.getElementById('rejectRate').textContent = decided > 0
        ? Math.round((rejected / decided) * 100) + '% rejection rate'
        : '- reject rate';
...
document.getElementById('submitAsk').addEventListener('click', ()=>{
    ...
    const fear = parseInt(document.getElementById('fearSlider').value);
    state.entries.push({
